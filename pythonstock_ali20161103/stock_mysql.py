...
for i in range(300000,302000):  
    ...
    if zhangfu > 9.8:
        print "%d,今天开盘价：%5.2f ,目前价：%5.2f,昨天收盘价：%5.2f,涨幅：%4.2f%%" %(i,\
opening_price,now_price,last_day_closed_price,zhangfu)
        num=num+1
            #print "%s,今天开盘价：%s ,目前价位：%s,昨天收盘价：%s,涨幅：%9.2f%%" %(name,o\pening_price,now_price,last_day_closed_price,zhangfu) 
        f = open("./stock3.txt", 'a')    
        print >> f, "%d,%5.2f ,%5.2f,%5.2f,涨幅：%4.2f%%" %(i,opening_price,now_price,\
last_day_closed_price,zhangfu)
        try:
            ...
            #print "insert into stock values(%s,%s,%s,%s,%s)" %(i,name2,opening_price,last_day_closed_price,now_price)
            cur.execute("insert into stock values(%s,%s,%s,%s,%s)" %(i,name2,\
opening_price,last_day_closed_price,now_price))
        ...
        #insert into stock values(300021,"大于", 14.73, 14.85, 16.34);
         #create table stock(id varchar(10) PRIMARY KEY,name varchar(20),opening_price varchar(10),last_day_closed_price varchar(10),now_price varchar(10));
 #drop tables stock;
